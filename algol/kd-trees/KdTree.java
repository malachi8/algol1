public class KdTree {
	...
	private Node insert(Node parent, Point2D p, RectHV rect, int level) {
		...
		if (level % 2 == 0) {
			if (p.x() < parent.p.x()) {
				parent.lb = insert(parent.lb, p, new RectHV(parent.rect.xmin(),
						parent.rect.ymin(), parent.p.x(), parent.rect.ymax()),
						nextLevel);
			} else {
				parent.rt = insert(parent.rt, p,
						new RectHV(parent.p.x(), parent.rect.ymin(),
			...
			if (p.y() < parent.p.y()) {
				...
				parent.rt = insert(parent.rt, p, new RectHV(parent.rect.xmin(),
						parent.p.y(), parent.rect.xmax(), parent.rect.ymax()),
						nextLevel);
			}
		}
	...
	private boolean contains(Node node, Point2D point) {
		...
		if (node.rect.contains(point)) {
			...
			// node.rect.contains(new Point2D(rect.xmax(), rect.ymax()))){
			if (node.lb != null) {
				if(contains(node.lb, point)){
					return true;
				}
			}
			if (node.rt != null) {
				if(contains(node.rt, point)){
					return true;
				}
			}
		}

		return node.p.equals(point);
	}
